...
Pages 不运行 Python、FastAPI 或 SQLite。发布前由本地解析器把真实安装转换成只读静态快照，前端的静态适配层在浏览器中完成目录读取、简繁/模糊/拼音搜索与补全、明确归属和无阵营筛选、事件筛选、排序、关联查询和详情请求。静态适配层会为后续新增的目录字段补齐安全默认值，使前端和固定数据在分阶段部署期间仍可正常读取声音等旧版目录。VXL 详情仍由 Three.js 交互渲染，SHP 与卡片使用按单位阵营预生成的玩家色 WebP，声音按需播放 24 kbit/s Opus。
...
- 启动目录、单位详情、动画 WebP、卡片图集与搜索图集优先走固定版本 jsDelivr，Pages 保留同源副本并仅在失败时回退，不用竞速请求制造双倍流量；
