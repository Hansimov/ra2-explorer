...
每个声音事件会建立语义兼容的动作池，并在行走、卧倒、匍匐、起身、站姿或卧姿开火、不同待机和欢呼等实际动作中优先选择尚未展示的序列；只有兼容动作均已使用时才复用。阵亡、空中坠落和翻滚阵亡序列只用于阵亡声音。事件文字的位置根据整段动画所有帧的不透明像素自动避让，保持靠近角色且不遮挡动作。
...
合成器输出带逐单位章节的步兵版视频，编码为 H.264 1080×1920、30 fps 和 AAC 48 kHz 双声道。录制逐条等待原始声音自然结束，相邻声音约保留 0.50 秒间隔，单位切换使用完整的淡出、横向滑动和淡入过场；不设置目标总时长，也不通过倍速或裁剪压缩内容。验收覆盖声音清单完整性、动作语义与覆盖度、事件文字避让、声音间隔、最后一条声音边界、页面与网络错误、CABLE 路由和时延、源分辨率、帧时钟、中英文文本、最终编码及章节数量。
