...
function sequenceAnimation(group, slot, sourceId, ordinal) {
  ...
  const frames = Array.from({ length: Math.min(24, Math.max(1, Number(sequence.frame_count) || 1)) }, (_, index) => {
    ...
    const shadowFrame = pairedShadow && frame < shadowOffset && frame + shadowOffset < visual.sourceFrameCount
      ? frame + shadowOffset
      : undefined;
    return assetPreviewUrl(sequence.assetId, frame, shadowFrame, 12, { paletteKind: sequence.palette || "unit" });
  });
  return {
...
function presentationHtml() {
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><style>
    ...
    *{box-sizing:border-box}html,body{width:100%;height:100%;margin:0;overflow:hidden}body{background:radial-gradient(circle at 50% 22%,#27292e 0,#121419 42%,#080a0d 78%)}
    .shell{display:grid;grid-template-rows:270px minmax(0,1fr) 64px;width:100%;height:100%;transition:opacity .28s ease}.carousel{display:grid;place-items:center;padding:16px 24px 8px;overflow:hidden}.unit-track{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1.5fr) minmax(0,1fr);align-items:end;gap:14px;width:100%;height:242px}.unit-peek,.unit-current{display:grid;grid-template-rows:auto auto;align-content:end;justify-items:center;min-width:0;text-align:center;transition:opacity .25s ease,transform .25s ease}.unit-peek{opacity:.24;transform:scale(.82);color:#a7adb7}.unit-peek img{visibility:hidden;width:86px;height:68px;margin-bottom:18px;object-fit:contain;image-rendering:pixelated}.unit-peek strong{max-width:100%;overflow:hidden;font-size:25px;font-weight:620;text-overflow:ellipsis;white-space:nowrap}.unit-current{position:relative;padding:4px 16px}.unit-current img{visibility:hidden;width:150px;height:118px;margin-bottom:27px;object-fit:contain;image-rendering:pixelated;filter:drop-shadow(0 12px 25px rgba(0,0,0,.58))}.unit-current strong{max-width:100%;overflow:hidden;color:#ef625c;font-size:52px;line-height:1.08;text-overflow:ellipsis;white-space:nowrap;text-shadow:0 8px 32px rgba(156,35,31,.32)}
    .content{display:grid;grid-template-rows:minmax(0,1fr) 600px;min-height:0;padding:0 40px 14px}.panel{min-height:0}.visual{position:relative;overflow:hidden;background:radial-gradient(circle at 50% 64%,rgba(139,42,38,.28),rgba(20,23,28,.16) 42%,transparent 76%)}.visual:before{position:absolute;inset:0;content:"";opacity:.1;background:repeating-linear-gradient(0deg,transparent 0,transparent 4px,rgba(255,255,255,.022) 5px);pointer-events:none}.stage-frame{position:absolute;inset:8px 38px 0;display:grid;place-items:center}.subject{width:100%;height:100%;object-fit:contain;image-rendering:pixelated;filter:drop-shadow(0 28px 25px rgba(0,0,0,.62));transform:translateY(150px) scale(1.46);transform-origin:center;transition:opacity .2s ease}.voice-head{position:absolute;right:38px;top:26px;z-index:2;display:flex;align-items:center;justify-content:flex-end}.event{display:inline-flex;align-items:center;color:#d5847f;font-size:23px;font-weight:680;letter-spacing:.05em;text-shadow:0 5px 18px rgba(0,0,0,.52)}.event i{display:none}
    .voice{display:grid;overflow:hidden;padding:0 42px 18px;transition:opacity .2s ease}.transcript{display:grid;align-content:start;justify-items:center;gap:27px;min-height:0;padding:30px 4px 0}.text-block{display:block;width:100%;text-align:center}.original,.localized{margin:0 auto;max-width:970px;overflow-wrap:anywhere;text-align:center;text-wrap:balance}.original{color:#f7f8fa;font-family:"Segoe UI","Microsoft YaHei UI",sans-serif;font-size:60px;font-weight:670;line-height:1.27;letter-spacing:.001em}.localized{color:#d9dde3;font-size:52px;font-weight:590;line-height:1.38}.text-block.hidden{display:none}
  ...
  </style></head><body><div class="shell"><header class="carousel"><div class="unit-track"><div class="unit-peek previous"><img alt=""><strong></strong></div><div class="unit-current"><img alt=""><strong></strong></div><div class="unit-peek next"><img alt=""><strong></strong></div></div></header><main class="content"><section class="panel visual"><div class="stage-frame"><img class="subject" alt="单位动画"></div><div class="voice-head"><span class="event"><i></i><b></b></span></div></section><section class="panel voice"><div class="transcript"><div class="text-block original-block"><p class="original"></p></div><div class="text-block localized-block"><p class="localized"></p></div></div></section></main><footer class="progress-shell"><div class="progress"><b></b></div></footer></div><div class="transition"><div class="transition-card"><small></small><h2></h2><p></p><div class="site"></div></div></div><audio id="voice-audio" preload="auto"></audio><script>
